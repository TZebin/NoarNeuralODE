...
class SurvNODE(nn.Module):
    ...
    def forward(self, x, tstart, tstop, from_state, to_state):
        ...
        S = torch.cat([S[i:i+1,from_state[i]-1,to_state[i]-1] for i in range(len(from_state))])
...
def loss(odesurv,x,Tstart,Tstop,From,To,trans,status,mu=1e-4):
    ...
    # print(odesurv.transition_matrix)
    ...
    S = odesurv(x,Tstart,Tstop,From,To)
    loglik = -torch.log(S).mean()#-(status*torch.log(lam)+torch.log(S)).mean()
